...
public class Day4 {
    ...
    public static void main(String[] args) {
        List<String> games = Utils.Files.readFileByLine("src/Day4/input.txt");
        // List<String> games = Utils.Files.readFileByLine("src/Day4/test.txt");

        List<Integer> scores = getScores(games);
    ...
    private static List<Integer> getScores(List<String> games) {
        List<Integer> scores = new ArrayList<Integer>();

        for (String game : games) {
            String[] gameAndCards = game.split(": ");
            String[] cards = gameAndCards[1].split(" \\| ");

            String[] winningNums = cards[0].split(" ");
            String[] myNums = cards[1].split(" ");
            System.out.println(Arrays.toString(winningNums) + "+" + Arrays.toString(myNums));

            scores.add(getScore(winningNums, myNums));
        }

        return scores;
    }

    private static int getScore(String[] winningNums, String[] myNums) {
        int wins = 0;
        for (String winningNum : winningNums) {
            try {
                int wn = Integer.parseInt(winningNum);
                for (String myNum : myNums) {
                    try {
                        int mn = Integer.parseInt(myNum);
                        if (wn == mn) {
                            wins++;
                        }
        ...
        return (wins == 0) ? 0 : (int) Math.pow(2, wins - 1);
    }
}
